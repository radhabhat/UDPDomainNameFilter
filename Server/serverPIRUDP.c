...
int passivesock(const char *service,const char *transport,int qlen)
{
        ...
        {
                printf("\nCan't get the %s service !!\n",service);
                exit(0);
        }
        /* Map protocol name to protocol number */
        if((ppe=getprotobyname(transport))== 0)
        {
                printf("\nCan't get the protocol !!\n");
                exit(0);
	 }
        /* use protocol to choose the socket type */
...
int main(int argc,char **argv)
{
	...
	for(;;)
	{
		...
		//RK//i = recv(connefd,&r1,sizeof(r1),0);
		i=recvfrom(listenfd,&r1,sizeof(r1),0,(struct sockaddr *)&cliaddr,&len);
